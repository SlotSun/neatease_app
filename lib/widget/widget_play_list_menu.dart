...
import 'widget_edit_play_list.dart';

class PlayListMenuWidget extends StatefulWidget {
  final UserOrderPlaylist _playlist;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Offstage(
            offstage: widget._playlist.creator.userId != widget._model.allPlayList.first.userId,
          ...
          _buildMenuItem('images/icon_del.png', '删除', () async {
            NetUtils().delPlayList( widget._playlist.id).then((v){
              if(v) Navigator.pop(context, widget._playlist.privacy);
              else Fluttertoast.showToast(msg:'删除失败，请重试');
            });
          }),
          Container(
            color: Colors.grey,
